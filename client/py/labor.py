...
# Helpers
def __create_request(method, *args, **kw):
    headers = {}
    headers.update(kw)
    d = {
...
def __normalize_action_name(method):
...
class Labor(object):
    ...
    def connect(self, addr, con_type):
        ...
        if con_type not in (TYPE_PUBSUB,):
    ...
    def use(self, method, *args, **kw):
        method = __normalize_action_name(method)
        req = __create_request(method, *args, **kw)
